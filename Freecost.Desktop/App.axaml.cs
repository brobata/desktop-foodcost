// Location: Freecost.Desktop/App.axaml.cs
...
public partial class App : Application
{
    ...
    public override void Initialize()
    {
        ...
        OfficeOpenXml.ExcelPackage.License.SetNonCommercialOrganization("Freecost");
    }
    ...
    public override async void OnFrameworkInitializationCompleted()
    {
        ...
        try
        {
            ...
            var dbContext = scope.ServiceProvider.GetRequiredService<FreecostDbContext>();
            ...
            var migrationSafety = Services?.GetService<Freecost.Data.Services.IMigrationSafetyService>();
        ...
        if (ApplicationLifetime is IClassicDesktopStyleApplicationLifetime desktop)
        {
            ...
            // Initialize session service to check if user previously signed in
            // logFile variable removed - using SafeFileLogger
            SafeFileLogger.Log("startup", "=== SESSION INITIALIZATION START ===");
            System.Diagnostics.Debug.WriteLine("[APP STARTUP] Initializing session service...");
            var sessionService = Services.GetRequiredService<IUserSessionService>();
            bool isAuthenticated = false;

            try
            {
                // Add timeout to prevent hanging on startup
                System.Diagnostics.Debug.WriteLine("[APP STARTUP] Calling InitializeAsync with 5 second timeout...");
                SafeFileLogger.Log("startup", "Calling session InitializeAsync...");
                var initTask = sessionService.InitializeAsync();
                var timeoutTask = Task.Delay(5000); // 5 second timeout
                var completedTask = await Task.WhenAny(initTask, timeoutTask);

                if (completedTask == initTask)
                {
                    isAuthenticated = await initTask;
                    System.Diagnostics.Debug.WriteLine($"[APP STARTUP] ✓ Session initialization completed. Authenticated: {isAuthenticated}");
                    SafeFileLogger.Log("startup", $"Session initialization completed - Authenticated: {isAuthenticated}");
                }
                else
                {
                    System.Diagnostics.Debug.WriteLine("[APP STARTUP] ⚠️ Session initialization TIMEOUT (5000ms) - continuing in offline mode");
                    SafeFileLogger.Log("startup", "Session initialization TIMEOUT");
                }
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"[APP STARTUP] ✗ Session initialization failed: {ex.Message} - continuing in offline mode");
                System.Diagnostics.Debug.WriteLine($"[APP STARTUP] Exception type: {ex.GetType().Name}");
                if (ex.InnerException != null)
                {
                    System.Diagnostics.Debug.WriteLine($"[APP STARTUP] Inner exception: {ex.InnerException.Message}");
                }
            }

            // CRITICAL: If user is authenticated (session restored from token),
            // sync restaurants immediately to clean up stale locations
            if (isAuthenticated)
            {
                try
                {
                    System.Diagnostics.Debug.WriteLine("[App.Startup] User authenticated - syncing restaurants to remove stale locations");
                    var syncService = Services.GetRequiredService<ISyncService>();
                    var syncResult = await syncService.SyncRestaurantsAsync();

                    if (syncResult.IsSuccess)
                    {
                        System.Diagnostics.Debug.WriteLine("[App.Startup] Restaurant sync completed - locations are up-to-date");
                    }
                    else
                    {
                        System.Diagnostics.Debug.WriteLine($"[App.Startup] Restaurant sync failed: {syncResult.ErrorMessage}");
                    }
                }
                catch (Exception syncEx)
                {
                    System.Diagnostics.Debug.WriteLine($"[App.Startup] Error syncing restaurants: {syncEx.Message}");
                    // Continue anyway - app can still work with cached data
                }

                // Note: Firebase migration to Supabase completed
            }

            // CRITICAL: Validate environment variables before continuing
            // If variables are set in registry but not loaded by the system yet, user needs to restart
            if (!ValidateEnvironmentVariables())
            {
                System.Diagnostics.Debug.WriteLine("[APP STARTUP] Environment variables not accessible - prompting user to restart");
                await ShowEnvironmentVariableRestartPrompt(desktop.MainWindow);
                return; // Exit startup - user will restart
            }
    ...
    private async Task UpdateLocationFromOnboarding()
    {
        try
        {
            ...
            Freecost.Core.Models.Location? offlineLocation = null;
            ...
            else
            {
                ...
                var newOfflineLocation = new Freecost.Core.Models.Location
                {
                    Id = Guid.NewGuid(),
    ...
    private async Task EnsureDefaultLocationAsync()
    {
        try
        {
            ...
            if (offlineLocation == null)
            {
                ...
                offlineLocation = new Freecost.Core.Models.Location
                {
                    Id = Guid.NewGuid(),
                    UserId = null, // NULL = offline-only location
                    Name = "My Restaurant",
    ...
    private async Task ShowEnvironmentVariableRestartPrompt(Window? parentWindow)
    {
        try
        {
            await Avalonia.Threading.Dispatcher.UIThread.InvokeAsync(async () =>
            {
                ...
                {
                    Title = "Restart Required - Freecost",
                    Width = 500,
                    Height = 250,
                    ...
                    Content = new StackPanel
                    {
                        ...
                        Children =
                        {
                            ...
                            new TextBlock
                            {
                                Text = "Freecost has been installed successfully, but your computer needs to restart for the application to work properly.",
    ...
    private void ConfigureServices(ServiceCollection services)
    {
        ...
        services.AddDbContext<FreecostDbContext>(options =>
        ...
        services.AddScoped<ILocationService, LocationService>(sp =>
            new LocationService(
                ...
                sp.GetService<SupabaseDataService>(),
        ...
        // Photo Service (Singleton - creates directories once)
        services.AddSingleton<IPhotoService>(sp => new PhotoService(sp.GetRequiredService<SupabasePhotoService>()));
        ...
        // ========================================
        // SUPABASE SERVICES (Build 0022)
        // ========================================

        // Supabase Data Service (Singleton - CRUD operations)
        services.AddSingleton<SupabaseDataService>();

        // Supabase Photo Service (Singleton - cloud storage with local caching)
        services.AddSingleton<SupabasePhotoService>();

        // Supabase Client (Singleton - shared client instance for all services)
        // CRITICAL: This must be registered before any service that depends on Supabase.Client (e.g., BugReportService)
        services.AddSingleton<Supabase.Client>(sp =>
        {
            return SupabaseClientProvider.GetClientAsync().GetAwaiter().GetResult();
        });

        // Supabase Realtime Service (Singleton - live updates via WebSockets)
        // services.AddSingleton<SupabaseRealtimeService>(); // TODO: Enable after fixing SocketResponseEventArgs
        ...
        // ========================================
        // SYNC SERVICE - SUPABASE (Build 0022)
        // ========================================

        // Supabase Sync Service (450 lines vs 2011 lines - 78% reduction!)
        // Uses SQL timestamp-based delta sync instead of complex tracking
        // Now includes photo sync to Supabase Storage
        services.AddSingleton<ISyncService>(sp => new SupabaseSyncService(
            ...
            sp.GetRequiredService<SupabasePhotoService>(),
            sp.GetService<ILogger<SupabaseSyncService>>()
        ));
        ...
        services.AddSingleton<Freecost.Data.Services.IMigrationSafetyService, Freecost.Data.Services.MigrationSafetyService>();
        ...
        // Bug Report Service (Singleton - submits bug reports to Supabase)
        services.AddSingleton<IBugReportService>(sp =>
        {
            return new BugReportService(
                ...
                sp.GetService<ILogger<BugReportService>>(),
                sp.GetService<IIngredientService>(),
                sp.GetService<IRecipeService>(),
                sp.GetService<IEntreeService>()
            );
        });
        ...
        services.AddScoped<IPhotoGalleryService, Freecost.Data.Services.PhotoGalleryService>();
