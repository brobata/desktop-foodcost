using FreecostUser = Freecost.Core.Models.User;
...
public class SupabaseAuthService : IAuthenticationService
{
    ...
    private FreecostUser? _currentUser;
    ...
    /// </summary>
    public async Task<FreecostUser> SignInAsync(string email, string password)
    {
        ...
        try
        {
            ...
            if (user == null)
            {
                ...
                user = new FreecostUser
                {
                    Id = Guid.NewGuid(),
                    Email = email,
                    SupabaseAuthUid = supabaseUid, // Using same field for Supabase UID
                    Role = isFirstUser ? Freecost.Core.Enums.UserRole.Admin : Freecost.Core.Enums.UserRole.Viewer,
                    IsActive = true,
            ...
            if (!user.IsActive)
            {
                Debug.WriteLine("[Auth] ERROR: FreecostUser account is disabled");
    ...
    /// </summary>
    public async Task<FreecostUser> SignUpAsync(string email, string password, string displayName)
    {
    ...
    /// </summary>
    public async Task<FreecostUser?> GetCurrentUserAsync()
    {
    ...
    /// </summary>
    public async Task<FreecostUser?> RestoreSessionAsync(string refreshToken, string? accessToken = null)
    {
